 ...
 */
public class CassandraClient10WithToken extends DB
{
...
//		String myhost = allhosts[random.nextInt(allhosts.length)];
		...
		//		}
		//TODO add by hxd
//		System.out.println("init cassandra connection");
		trs=new TTransport[allhosts.length];
		clients=new Cassandra.Client[allhosts.length];
		for(int i=0;i<allhosts.length;i++){
			for (int retry = 0; retry < ConnectionRetries; retry++)
			{
				trs[i] = new TFramedTransport(new TSocket(allhosts[i], 9160));
				TProtocol proto = new TBinaryProtocol(trs[i]);
				clients[i] = new Cassandra.Client(proto);
				try
				{
					trs[i].open();
					System.out.println("init cassandra connection for "+allhosts[i]);
					connectexception = null;
					break;
				} catch (Exception e)
				{
			...
			{
				System.err.println("Unable to connect to " + allhosts[i] + " after " + ConnectionRetries
						+ " tries");
	...
	public int read(String table, String key, Set<String> fields, HashMap<String, ByteIterator> result)
	{
		if (!_table.equals(table)) {
			try
			{
				clients[LocateKey.getOnlyIp(key)].set_keyspace(table);
				_table = table;
			}
			catch (Exception e)
			{
		...
		for (int i = 0; i < OperationRetries; i++)
		{
			...
			try
			{
				...
				List<ColumnOrSuperColumn> results = clients[LocateKey.getOnlyIp(key)].get_slice(ByteBuffer.wrap(key.getBytes("UTF-8")), parent, predicate, readConsistencyLevel);
			...
			Vector<HashMap<String, ByteIterator>> result)
	{
		if (!_table.equals(table)) {
			try
			{
				clients[LocateKey.getOnlyIp(startkey)].set_keyspace(table);
				_table = table;
			}
			catch (Exception e)
			{
		...
		for (int i = 0; i < OperationRetries; i++)
		{
			...
			try
			{
				...
				List<KeySlice> results = clients[LocateKey.getOnlyIp(startkey)].get_range_slices(parent, predicate, kr, scanConsistencyLevel);
	...
	public int insert(String table, String key, HashMap<String, ByteIterator> values)
	{
		...
		for (int i = 0; i < OperationRetries; i++)
		{
			...
			try
			{
				...
				clients[LocateKey.getOnlyIp(key)].batch_mutate(record, writeConsistencyLevel);
	...
	public int delete(String table, String key)
	{
		...
		for (int i = 0; i < OperationRetries; i++)
		{
			try
			{
				clients[LocateKey.getOnlyIp(key)].remove(ByteBuffer.wrap(key.getBytes("UTF-8")),
	...
	{
		CassandraClient10WithToken cli = new CassandraClient10WithToken();
